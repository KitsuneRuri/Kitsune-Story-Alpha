--[[
    ...
    -- SkeletonMovingState Class --

    
]]

SkeletonMovingState = Class{__includes = BaseState}

function SkeletonMovingState:init(entity, player)
	self.entity = entity
    self.player = player
    self.map = entity.map
...
function SkeletonMovingState:update(dt)
    ...
    if self.movingTimer > self.movingDuration then
        ...
        if math.random(4) == 1 then
            self.entity:changeState('idle', {
                ...
                -- random amount of time for skeleton to be idle
                wait = math.random(5)
            })
        else
            self.movingDirection = math.random(2) == 1 and 'left' or 'right'
    ...
    elseif self.entity.direction == 'left' then
        self.entity.x = self.entity.x - SKELETON_MOVE_SPEED * dt
